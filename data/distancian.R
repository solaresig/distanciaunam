...
for (i in 1:nrow(total2)){
  ...
  # rowcoords[,j]<-rowcoords[,j]*ca$eig[j,1]
  #}
...
write_csv(tableL, "/sin_n/tableL.csv")
...
narrative1<-narrative.group(tableU, tableU$clusters, lim=200000, phase=tableU$year,type.calc="level", parallel = T)
...
tableU$id<-tableU$point
tableU$year<-tableU$year %>% as.character() %>% as.numeric()
narrative1<-narrative.group(tableU, tableU$clusters, lim=200, phase=tableU$year,type.init="distribution", parallel=T)
narrative<-correct.group(narrative1, narrative1$clusters, 1000, direction = "forward", type.calc = "average", parallel=T)
write_csv(narrative, "narrative2.csv")
...
ggplot(tableL, aes(x=year, y=pervariance, fill=dim))+
  geom_point()+ geom_line()+ geom_label(aes(label=rows))+
  theme_minimal()+
